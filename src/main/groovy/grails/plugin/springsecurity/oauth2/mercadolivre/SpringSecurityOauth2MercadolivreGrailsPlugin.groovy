...
class SpringSecurityOauth2MercadolivreGrailsPlugin extends Plugin {
    ...
    def title = "Spring Security Oauth2 Mercadolivre" // Headline display name of the plugin
...
'''
    ...
    def documentation = "http://grails.org/plugin/spring-security-oauth2-mercadolivre"
...
//    def scm = [ url: "http://svn.codehaus.org/grails-plugins/" ]
    ...
    Closure doWithSpring() { {->
        ...
        if (!hasProperty('log')) {
            log = LoggerFactory.getLogger(SpringSecurityOauth2MercadolivreGrailsPlugin)
        }

        if (printStatusMessages) {
            println("Configuring Spring Security OAuth2 mercado livre plugin...")
        }
        SpringSecurityUtils.loadSecondaryConfig('DefaultOAuth2MercadoLivreConfig')
        if (printStatusMessages) {
            println("... finished configuring Spring Security Mercado Livre\n")
        }
        }
    }
